...
# Monkey-patch torchaudio for s3prl compatibility (newer torchaudio removed these APIs)
import torchaudio
if not hasattr(torchaudio, "set_audio_backend"):
    torchaudio.set_audio_backend = lambda *a, **kw: None
if "torchaudio.sox_effects" not in sys.modules:
    _sox_mod = types.ModuleType("torchaudio.sox_effects")
    _sox_mod.apply_effects_tensor = lambda tensor, sr, effects: (tensor, sr)
    sys.modules["torchaudio.sox_effects"] = _sox_mod
    torchaudio.sox_effects = _sox_mod
...
class ECAPA_TDNN(nn.Module):
    def __init__(
        ...
        # Use local s3prl if S3PRL_LOCAL_PATH is set and valid, otherwise auto-download
        local_s3prl_path = os.environ.get("S3PRL_LOCAL_PATH", "")
        if local_s3prl_path and os.path.exists(os.path.join(local_s3prl_path, "hubconf.py")):
